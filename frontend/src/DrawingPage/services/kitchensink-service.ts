...
class KitchenSinkService {
    ...
    initializePaper() {
        ...
        const paper = this.paper = new joint.dia.Paper({
            ...
            defaultLink: new appShapes.app.Link() as joint.dia.Link,
        ...
        paper.on('blank:contextmenu', (evt) => {
            this.renderContextToolbar({ x: evt.clientX as number, y: evt.clientY as number });
        });

        paper.on('cell:contextmenu', (cellView, evt) => {
            this.renderContextToolbar({ x: evt.clientX as number, y: evt.clientY as number }, [cellView.model]);
        });
    ...
    initializeSelection() {
        ...
        this.selection.on('selection-box:pointerup', (elementView: joint.dia.ElementView, evt: joint.dia.Event) => {
            ...
            if (evt.button === 2) {
                evt.stopPropagation();
                this.renderContextToolbar({ x: evt.clientX as number, y: evt.clientY as number }, this.selection.collection.toArray());
            }
    ...
    selectPrimaryCell(cellView: joint.dia.CellView) {
        ...
        if (cell.isElement()) {
            this.selectPrimaryElement(cellView as joint.dia.ElementView);
        } else {
            this.selectPrimaryLink(cellView as joint.dia.LinkView);
        }
    ...
    applyOnSelection(method: string) {
        ...
        this.selection.collection.models.forEach(function(model: joint.dia.Cell) { model[method as keyof typeof model](); });
